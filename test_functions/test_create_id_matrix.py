...
def create_id_matrix(constraints: np.ndarray, dofs_per_nodes: int, NumNodes: int) -> Tuple[np.ndarray, int]:
  ...
  # Checking node duplicacy
  node_mum = constraints[:, 0] 
  unique_nodes = np.unique(node_mum)

  if len(unique_nodes) != len(node_mum):
      raise ValueError("Error: Duplicate node numbers found in the constraints.")
...
def test_createID_matrix():
    ...
    Global_ID, NEqns = create_id_matrix(constraints, dofs_per_nodes, NumNodes)
    print(Global_ID)
    print(NEqns)

if __name__ == "__main__":
    test_createID_matrix()
